...
func getInputForLookup(format, name, uri, dir string) lib.InputConverter {
	...
	case strings.ToLower(maxmind.TypeGeoLite2CountryMMDBIn):
		input = maxmind.NewGeoLite2CountryMMDBIn(
			...
			lib.ActionAdd,
			maxmind.WithGeoLite2CountryMMDBInURI(maxmind.TypeGeoLite2CountryMMDBIn, uri),
		)
	...
	case strings.ToLower(maxmind.TypeDBIPCountryMMDBIn):
		input = maxmind.NewGeoLite2CountryMMDBIn(
			...
			lib.ActionAdd,
			maxmind.WithGeoLite2CountryMMDBInURI(maxmind.TypeDBIPCountryMMDBIn, uri),
		)
	...
	case strings.ToLower(maxmind.TypeIPInfoCountryMMDBIn):
		input = maxmind.NewGeoLite2CountryMMDBIn(
			...
			lib.ActionAdd,
			maxmind.WithGeoLite2CountryMMDBInURI(maxmind.TypeIPInfoCountryMMDBIn, uri),
		)

	case strings.ToLower(mihomo.TypeMRSIn):
		input = mihomo.NewMRSIn(
			lib.ActionAdd,
			mihomo.WithMRSInNameAndURI(name, uri),
			mihomo.WithMRSInInputDir(dir),
		)
	...
	case strings.ToLower(plaintext.TypeTextIn):
		input = plaintext.NewTextIn(
			...
			lib.ActionAdd,
			plaintext.WithTextInInputDir(plaintext.TypeTextIn, dir),
			plaintext.WithTextInNameAndURI(plaintext.TypeTextIn, name, uri),
		)
	...
	case strings.ToLower(plaintext.TypeClashRuleSetIPCIDRIn):
		input = plaintext.NewTextIn(
			...
			lib.ActionAdd,
			plaintext.WithTextInInputDir(plaintext.TypeClashRuleSetIPCIDRIn, dir),
			plaintext.WithTextInNameAndURI(plaintext.TypeClashRuleSetIPCIDRIn, name, uri),
		)
	...
	case strings.ToLower(plaintext.TypeClashRuleSetClassicalIn):
		input = plaintext.NewTextIn(
			...
			lib.ActionAdd,
			plaintext.WithTextInInputDir(plaintext.TypeClashRuleSetClassicalIn, dir),
			plaintext.WithTextInNameAndURI(plaintext.TypeClashRuleSetClassicalIn, name, uri),
		)
	...
	case strings.ToLower(plaintext.TypeSurgeRuleSetIn):
		input = plaintext.NewTextIn(
			...
			lib.ActionAdd,
			plaintext.WithTextInInputDir(plaintext.TypeSurgeRuleSetIn, dir),
			plaintext.WithTextInNameAndURI(plaintext.TypeSurgeRuleSetIn, name, uri),
		)
