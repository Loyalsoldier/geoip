...
func newTextOut(iType, iDesc string, action lib.Action, opts ...lib.OutputOption) lib.OutputConverter {
	t := &text_out{
		Type:        iType,
		Action:      action,
		Description: iDesc,
...
func WithOutputExtension(extension string) lib.OutputOption {
	return func(t lib.OutputConverter) {
		if extension = strings.TrimSpace(extension); extension != "" {
			t.(*text_out).OutputExt = extension
		}
	}
}
...
func newTextOutFromBytes(iType, iDesc string, action lib.Action, data []byte) (lib.OutputConverter, error) {
	var tmp struct {
	...
	return newTextOut(
		...
		WithOutputExtension(tmp.OutputExtension),
